  ...
  @foreach($brandData as $key => $brand)
    <section class="brand-section">
      <div class="d-flex align-items-center justify-content-between mb-4 pb-3 border-bottom" 
           style="border-color: {{ ['nails' => '#E91E63', 'hair' => '#9C27B0', 'glow' => '#FF9800'][$brand['class']] }};">
        <div class="d-flex align-items-center gap-3">
          <div class="brand-icon-small {{ $brand['class'] }}">
            <i class="{{ $brand['icon'] }}"></i>
          </div>
          <div>
            <h4 class="mb-0 fw-bold">{{ $brand['label'] }}</h4>
            <small class="text-muted">{{ $brand['title'] }}</small>
          </div>
        </div>
        <a href="{{ route($brand['route']) }}" class="btn btn-view-brand {{ $brand['class'] }} btn-sm rounded-pill px-4">
          Vezi serviciile →
        </a>
      </div>
      
      <div class="row g-2">
        @forelse($gallery[$key] as $item)
        ...
        @empty
          <div class="col-12">
            <div class="text-center py-4">
              <i class="{{ $brand['icon'] }} fa-2x text-muted mb-3" style="opacity: 0.3;"></i>
